...
const BulkUploadDialog = ({ open, onClose, onSuccess }) => {
  ...
  const handlePreview = async () => {
    ...
    setLoading(true);
    try {
      const response = await productService.bulkPreview(file, updateExisting);
          ...
          <Box>
            <Alert severity="info" sx={{ mb: 3 }}>
              ...
              <Typography variant="caption" component="div">
                ...
                <br />
                2. Complete los datos de los productos
                <br />
            ...
            <Box sx={{ mt: 3 }}>
              <label>
                ...
                <Typography variant="body2" component="span">
                  Actualizar productos existentes (basado en código)
                </Typography>
              </label>
            </Box>
          </Box>
              ...
              <>
                <Alert severity={preview.summary.invalidRows > 0 ? 'warning' : 'success'} sx={{ mb: 3 }}>
                  ...
                  <Box sx={{ mt: 1 }}>
                    ...
                    <Chip
                      label={`${preview.summary.willCreate} crear`}
                      color="primary"
                      size="small"
                      sx={{ mr: 1, mb: 1 }}
                    />
                    {preview.summary.willUpdate > 0 && (
                      <Chip
                        label={`${preview.summary.willUpdate} actualizar`}
                        color="info"
                        size="small"
                        sx={{ mb: 1 }}
                      />
                ...
                {preview.preview.toCreate.length > 0 && (
                  <Box sx={{ mb: 2 }}>
                    ...
                    <Paper sx={{ mt: 1, maxHeight: 200, overflow: 'auto' }}>
                      <List dense>
                        {preview.preview.toCreate.map((product, idx) => (
                          <ListItem key={idx}>
                            <ListItemIcon>
                            ...
                            <ListItemText
                              ...
                              secondary={`Código: ${product.sku} - Precio: $${product.price}`}
                            />
                          </ListItem>
                        ))}
                      </List>
                ...
                {preview.preview.toUpdate.length > 0 && (
                  <Box>
                    ...
                    <Paper sx={{ mt: 1, maxHeight: 200, overflow: 'auto' }}>
                      <List dense>
                        {preview.preview.toUpdate.map((product, idx) => (
                          <ListItem key={idx}>
                            <ListItemIcon>
